...
const getSkillsByServiceId = (req, res, next) => {
  ...
  db.any(
    ...
    `SELECT *, skills.id AS skills_id, skills.name AS skills_name, services.name AS service_name FROM skills JOIN services ON services.id=skills.service_id WHERE service_id = $1`, [service_id]
  )
  .then(data => {
    res.status(200).json({
      status: 'success',
